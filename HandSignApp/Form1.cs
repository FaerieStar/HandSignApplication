...
namespace HandSignApp
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            //Debug.WriteLine(project_path + "\\Dataset\\Train\\train.csv");
        }
        ...
        private void create_model_btn_Click(object sender, EventArgs e)
        {
            ...
            net = new NeuralNet(isize, hsize, osize);
        ...
        private void train_btn_Click(object sender, EventArgs e)
        {
            ...
            for (int x = 0; x < epoch; x++)
            {
                ...
                using (StreamReader csvReader = new StreamReader(project_path + "\\Dataset\\train.csv"))
                //using (StreamReader csvReader = new StreamReader(dataset_directory + "/0Train/train.csv"))
                {
                    ...
                    while (!csvReader.EndOfStream)
                    {
                        ...
                        for (int i = 1; i < values.Length; i++)
                            net.setInputs((i - 1), Convert.ToDouble(values[i]));
        ...
        private void test_btn_Click(object sender, EventArgs e)
        {
            ...
            for (int x = 0; x < bm.Height; x++)
            {
                for (int y = 0; y < bm.Width; y++)
                {
                    ...
                    net.setInputs(y + (x * 28), grayscale_intensity);
                }
            }
            net.run();
            ...
            for (int i = 0; i < 24; i++)
            {
                curr = net.getOuputData(i);
                result += "Output[" + (i + 1) + "]: " + curr + "\n";
                if (curr > max)
                {
                    max = curr;
                    indexmax = i + 1;
            ...
            //result += "Output[" + (i + 1) + "]: " + net.getOuputData(i) + "\n";
            if (indexmax < 10)
            {
                output_show_txt.Text = Convert.ToString(Convert.ToChar(64 + indexmax));
            }
            else
            {
                output_show_txt.Text = Convert.ToString(Convert.ToChar(65 + indexmax));
            }
